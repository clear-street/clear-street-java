package com.clear_street.api.core
...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { ClearStreetInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ClearStreetInvalidDataException("`$name` is not set")
            is JsonNull -> throw ClearStreetInvalidDataException("`$name` is null")
            else -> throw ClearStreetInvalidDataException("`$name` is invalid, received $this")
        }
