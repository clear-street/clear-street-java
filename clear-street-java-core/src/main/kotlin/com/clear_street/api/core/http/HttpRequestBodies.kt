...
import com.clear_street.api.core.MultipartField
...
private fun serializePart(name: String, node: JsonNode): Sequence<Pair<String, InputStream>> =
    when (node.nodeType) {
        ...
        JsonNodeType.ARRAY ->
            node.elements().asSequence().flatMapIndexed { index, element ->
                serializePart("$name[$index]", element)
            }
        JsonNodeType.OBJECT ->
        ...
        JsonNodeType.POJO,
        null -> throw ClearStreetInvalidDataException("Unexpected JsonNode type: ${node.nodeType}")
    }
