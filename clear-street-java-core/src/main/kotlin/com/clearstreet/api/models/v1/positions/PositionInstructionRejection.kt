 ...
 *
 * Populated on the submit and cancel responses for a row rejected with a structured reason. Branch
 * on `reason` for programmatic handling and render your own copy; `rejection_reason` remains the
 * human-readable fallback and is the field to use when listing historical instructions.
 */
...
private constructor(
    ...
    /**
     * Reason-specific parameters as string key/value pairs (e.g. `available` / `requested`,
     * `expiry` / `business_date`, `required_level` / `account_level`). May be empty.
     *
     * This arbitrary value can be deserialized into a custom type using the `convert` method:
     * ```java
     * MyClass myObject = positionInstructionRejection.metadata().convert(MyClass.class);
     * ```
     */
    ...
    internal fun validity(): Int =
        (if (domain.asKnown().isPresent) 1 else 0) + (if (reason.asKnown().isPresent) 1 else 0)
